...
function createLog(action, actor='System', note=''){
  return { at: now(), action, actor, note };
}
...
$('#prForm').addEventListener('submit', (event) => {
  ...
  const pr = {
    ...
    nextApprover: form.get('nextApprover'),
    status: autoStatus(amount, form.get('nextApprover')),
    createdAt: now(),
    logs: [createLog('Created PR', form.get('requester'), 'ส่งเอกสารเข้าสู่ workflow')]
  };
  saveData([pr, ...loadData()]);
  event.target.reset();
});
...
$('#clearBtn').addEventListener('click', () => {
  if(confirm('ล้างข้อมูลทั้งหมดใน browser นี้?')) saveData([]);
});
...
function act(id, action){
  ...
  if(action === 'reject') {
    ...
    pr.logs.push(createLog('Rejected', actor, note));
  }
  if(action === 'close') {
    pr.status = STATUS.CLOSED;
    pr.logs.push(createLog('Closed after purchase', actor, note));
  }
  if(action === 'reset') {
    pr.status = autoStatus(pr.amount, pr.nextApprover);
    pr.logs.push(createLog('Reset workflow', actor, note));
  }
  saveData(data);
}
...
function showDetail(id){
  ...
  $('#modalBody').innerHTML = `
    <p><b>ผู้ขอ:</b> ${escapeHtml(pr.requester)} · <b>แผนก:</b> ${escapeHtml(pr.department)}</p>
    <p><b>Vendor:</b> ${escapeHtml(pr.vendor)} · <b>ยอดเงิน:</b> ${money(pr.amount)}</p>
    <p><b>รายการ:</b><br>${escapeHtml(pr.item)}</p>
    <p><b>เหตุผล:</b><br>${escapeHtml(pr.reason)}</p>
    <h3>ประวัติเอกสาร</h3>
    <div class="timeline">${pr.logs.map(log => `<div class="log"><b>${escapeHtml(log.action)}</b><div class="small">${escapeHtml(log.at)} · ${escapeHtml(log.actor)}</div><div>${escapeHtml(log.note)}</div></div>`).join('')}</div>
  `;
...
function renderStats(data){
  ...
  const waiting = data.filter(pr => pr.status.includes('Waiting') || pr.status === STATUS.SUBMITTED).length;
  const approved = data.filter(pr => pr.status === STATUS.APPROVED).length;
  const rejected = data.filter(pr => pr.status === STATUS.REJECTED).length;
...
function render(){
  ...
  $('#prList').innerHTML = filtered.length ? filtered.map(pr => `
    <article class="pr">
      <div class="pr-top">
        <div>
          <h3>${escapeHtml(pr.no)}</h3>
          <div class="meta">
            <span>${escapeHtml(pr.requester)}</span>
            <span>·</span>
            <span>${escapeHtml(pr.department)}</span>
            <span>·</span>
            <span>${escapeHtml(pr.vendor)}</span>
          </div>
        </div>
        <span class="pill ${badgeClass(pr.status)}">${escapeHtml(pr.status)}</span>
      </div>
      <div>${escapeHtml(pr.item)}</div>
      <div class="amount">${money(pr.amount)}</div>
      <div class="actions">
        <button class="ghost" onclick="showDetail('${pr.id}')">ดูรายละเอียด</button>
        <button class="secondary" onclick="act('${pr.id}','approve')">อนุมัติ/ขั้นถัดไป</button>
        <button class="danger" onclick="act('${pr.id}','reject')">ไม่อนุมัติ</button>
        <button class="ghost" onclick="act('${pr.id}','close')">ปิดงาน</button>
        <button class="ghost" onclick="act('${pr.id}','reset')">เริ่ม workflow ใหม่</button>
      </div>
    </article>
  `).join('') : '<div class="empty">ยังไม่มีเอกสาร PR หรือไม่พบผลการค้นหา</div>';
}
...
function seedData(){
  const sample = [
    ...
    id: crypto.randomUUID(), no: `PR-${new Date().getFullYear()}-${String(index+1).padStart(4,'0')}`,
    ...item, status: autoStatus(item.amount, item.nextApprover), createdAt: now(),
    logs: [createLog('Created sample PR', item.requester, 'ข้อมูลตัวอย่างสำหรับทดลองระบบ')]
  }));
  saveData(sample);
}
...
function exportCSV(){
  ...
  a.href = url;
  a.download = `pr-export-${new Date().toISOString().slice(0,10)}.csv`;
  a.click();
